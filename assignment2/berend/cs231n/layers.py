...
def boost_forward(x, boost_param):
    """
    Performs the forward pass for (inverted) dropout.
    ...
    - cache: A tuple (dropout_param, mask). In training mode, mask is the dropout
      mask that was used to multiply the input; in test mode, mask is None.
    """
    p, mode, boosting = boost_param['p'], boost_param['mode'], boost_param['boosting']
    if 'seed' in boost_param:
        np.random.seed(boost_param['seed'])
    ...
    cache = (boost_param, mask)
    out = out.astype(x.dtype, copy=False)

    return out, cache


def boost_backward(dout, cache):
    """
    Perform the backward pass for (inverted) dropout.
    ...
    dropout_param, mask = cache
    mode = dropout_param['mode']
    boosting = dropout_param['boosting']
...
def conv_backward_naive(dout, cache):
    ...
    H_out = 1 + (DH + 2 * pad - HH) / stride
    W_out = 1 + (DW + 2 * pad - WW) / stride
