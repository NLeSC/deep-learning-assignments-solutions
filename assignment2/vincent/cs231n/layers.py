...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = dout - dout.sum(axis=0) / n - shifted_x * (dout * shifted_x).sum(axis=0) / (
        n * np.square(varepssqrt))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #############################################################################
    # padd the image volume, which means adding zeros around every matrix of HxWs
    # num_npad = conv_param['pad']
    # stride = conv_param['stride']
    # npad = ((0,0),(0,0),(num_npad,num_npad),(num_npad,num_npad))
    # xpadded = np.pad(x, pad_width=npad, mode='constant', constant_values=0)
    #
    # # dimentions
    # N, C, H, W = x.shape
    # F, C, HH, WW = w.shape
    # Ho = 1 + (H + 2 * num_npad - HH) / stride
    # Wo = 1 + (W + 2 * num_npad - WW) / stride
    # out = np.zeros((N, F, Ho, Wo))
    # #forwared pass
    # for n in range(N): # images
    #   for f in range(F): # filters
    #       for c in range(C): # colors/channel
    #           for ho in range(Ho): #rows along Height
    #               for wo in range(Wo): # columns along width (not weights)
...
def spatial_batchnorm_forward(x, gamma, beta,
                              bn_param):
    """
...
def svm_loss(x, y):
    ...
    """
    N = x.shape[0]
    correct_class_scores = x[
        np.arange(N), y]
    margins = np.maximum(0,
                         x - correct_class_scores[
                             :,
                             np.newaxis] + 1.0)
