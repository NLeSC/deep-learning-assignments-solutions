...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for layer in xrange(1, self.num_layers + 1):
            self.params[
                'W%i' % layer] = np.random.normal(loc=0, scale=weight_scale,
    ...
    def loss(self, X, y=None):
        ...
        for layer in xrange(1, self.num_layers):
            ...
            if self.use_batchnorm:
                ...
                inp, cachei = affine_norm_relu_forward(
                    inp, Wi, bi, gammai, betai, bn_paramsi)
            else:
        ...
        for layer in xrange(self.num_layers - 1, 0, -1):
            ...
            if self.use_batchnorm:
                dl, dw, db, dgamma, dbeta = affine_norm_relu_backward(
                    dl, cache[layer])
