...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  image_term = x.dot(Wx)
  state_term = prev_h.dot(Wh)
  sum = image_term + state_term + b
  next_h = np.tanh(sum)
...
def rnn_step_backward(dnext_h, cache):
  ...
  - dWx: Gradients of input-to-hidden weights, of shape (N, H) not correct!
  ...
  dWx = x.transpose().dot(dhraw)
  dWh = prev_h.transpose().dot(dhraw)
  dx = dhraw.dot(Wx.transpose())
  dprev_h = dhraw.dot(Wh.transpose())
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in range(T):
    xt = x[:, t, :]
    h_next, step_cache[t] = rnn_step_forward(xt, prev_h, Wx, Wh, b)
    h[:, t, :] = h_next
    prev_h = h_next
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = (tanh_next_c, o, prev_c, f, i, g, prev_h, x, Wx, Wh )
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  tanh_next_c, o, prev_c, f, i, g, prev_h, x, Wx, Wh = cache
  dnext_c += o * (1 - tanh_next_c ** 2) * dnext_h
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in range(T):
    xt = x[:, t, :]
    next_h, next_c, step_cache[t] = lstm_step_forward(xt, prev_h, prev_c, Wx, Wh, b)
    h[:, t, :] = next_h
    prev_h = next_h
    prev_c = next_c
  cache = (step_cache, D)
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(range(T)):
    dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dprev_h, dprev_c, step_cache[t])
    dx[:, t, :] = dx_t
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
